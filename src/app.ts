...
// Declaração de tipos para a biblioteca global Supabase injetada via CDN
interface Window {
  supabase?: {
    createClient: (url: string, key: string) => SupabaseClientInstance;
  };
}
...
interface LeadData {
  ...
  gargalo: string;
  origem: string;
  status: 'NOVO' | 'EM_ANALISE' | 'CONVERTIDO';
  metadata: {
    user_agent: string;
    timestamp: string;
    pagina: string;
...
function initThemeToggle(): void {
  ...
  const savedTheme = (localStorage.getItem('sousza_theme') || localStorage.getItem('asza_theme') || 'dark') as ThemeMode;
  ...
  if (toggleBtn) {
    toggleBtn.addEventListener('click', () => {
      const activeTheme = (document.documentElement.getAttribute('data-theme') || 'dark') as ThemeMode;
...
function updateThemeIcon(btn: HTMLButtonElement | null, theme: ThemeMode): void {
  if (!btn) return;
...
function initContactForm(): void {
  ...
  form.addEventListener('submit', async (e: Event) => {
    ...
    const companyInput = document.getElementById('form-company') as HTMLInputElement | null;
    const interestInput = document.getElementById('form-interest') as HTMLSelectElement | null;
...
/* ==========================================================================
   ...
   ========================================================================== */
function initActiveNavSpy(): void {
